...
// Configura CORS para permitir solicitudes desde tus dominios de Netlify
const corsOptions = {
    origin: [
...
async function leerConfiguracion() {
    try {
        ...
        return { tasa_dolar: 0, pagina_bloqueada: false, fecha_sorteo: null }; // Valores por defecto incluyendo fecha_sorteo
    }
}
...
app.put('/api/admin/configuracion', async (req, res) => {
    ...
    config.fecha_sorteo = fecha_sorteo !== undefined ? fecha_sorteo : config.fecha_sorteo; // Guardar fecha del sorteo
